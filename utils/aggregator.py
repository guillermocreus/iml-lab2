...
class Aggregator:
	...
	def __init__(self):
		self._transformed_data = {'pca': {}, 'umap': {}, 'pca_sklearn': {}}
		self._results = {'complete': {}, 'pca': {}, 'umap': {}, 'pca_sklearn': {}}
		self._metrics = {'complete': {}, 'pca': {}, 'umap': {}, 'pca_sklearn': {}}
	...
	def evaluate(self, data, desired_components, y, dataset_name=''):

		results_umap = self.fit_UMAP(data, y, dataset_name=dataset_name)
		...
		for n_cluster in range(2, 10):
			...
			sh_score_pca_sklearn = silhouette_score(self._transformed_data['pca_sklearn'],
													labels_pca_sklearn)
			calinski_harabasz_score_pca_sklearn = calinski_harabasz_score(self._transformed_data['pca_sklearn'],
																		  labels_pca_sklearn)
			davies_bouldin_score_pca_sklearn = davies_bouldin_score(self._transformed_data['pca_sklearn'],
																	labels_pca_sklearn)
			...
			sh_score_umap = silhouette_score(self._transformed_data['umap'],
													labels_umap)
			calinski_harabasz_score_umap = calinski_harabasz_score(self._transformed_data['umap'],
																		  labels_umap)
			davies_bouldin_score_umap = davies_bouldin_score(self._transformed_data['umap'],
																	labels_umap)
